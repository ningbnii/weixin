...
class Wechat {
        ...
        if($token){
            ...
            if(IS_GET){
                ...
                if($this->encrypt_type == 'aes'){
                	...
                	$errorCode = $this->pc->decryptMsg($this->msg_signature,$this->timestamp, $this->nonce, $xml, $msg);
    ...
    public function response($content, $type = self::MSG_TYPE_TEXT){
        ...
        if($this->encrypt_type == 'aes'){
            $encryptMsg = '';  //加密后的密文
            $errorCode = $this->pc->encryptMsg($result, $this->timestamp, $this->nonce, $encryptMsg);
            if($errorCode == 0){
                $result = $encryptMsg;
            }else{
                echo $errorCode;
     ...
     */
    public function replyMusic($title, $discription, $musicurl, $hqmusicurl){
